"""
Complete training script for TinyZero with DDP parallelization
Based on original train.py - ONLY ADDS DDP wrapper for multi-GPU training
"""
import argparse
import yaml
import torch
...
class Trainer:
    ...
    def __init__(self, config: dict, output_dir: str):
        ...
        # ========== DDP WRAPPING - ONLY ADDITION ==========
        # Wrap policy model with DDP if in distributed mode
        if dist.is_available() and dist.is_initialized():
            ...
            if rank == 0:
                print()
                print("=" * 80)
                print("APPLYING DDP PARALLELIZATION")
                print("=" * 80)
                ...
                print()
                print("   Wrapping policy model with DDP...")

            # Wrap policy model with DDP - simple and reliable
            ...
            if rank == 0:
                print("   Policy model wrapped with DDP")
                print()
                ...
                print(f"   - GPU {rank}: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved")
                print()
                print("=" * 80)
                print()
        # ====================================================
    ...
    def train(self):
        ...
        self.log(f"Total training time: {hours}h {minutes}m {seconds}s ({total_time_seconds:.1f}s)")
        self.log("="*60)
...
def main():
    ...
    # ========== DDP DISTRIBUTED INITIALIZATION - ONLY ADDITION ==========
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        if rank == 0:
            print()
            print("=" * 80)
            print("DDP DISTRIBUTED TRAINING INITIALIZED")
            print("=" * 80)
